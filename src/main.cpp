...
uint256 hashGenesisBlock("0x69efd2b7b7be5f555c0d87e2047226cbf481d666e4b4883ea2553d5d605228c1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Experiencecoin: starting difficulty is 1 / 2^12
...
{
    // Experiencecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 8600 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 5256000); // Experiencecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60; // Experiencecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Experiencecoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Experiencecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Experiencecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5904f844b7b626f1d821dcffe40a26f4a51d5462abc52f65c7449ecaf0e6c501");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 03/Feb/2014 Philip Seymour Hoffman, Actor of Depth, Dies at 46";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391456897;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087091571;

        if (fTestNet)
        {
            block.nTime    = 1391456890;
            block.nNonce   = 386405748;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd27e6ed54206538c5b0570d859b8b87fff938476a5cde4d52f2e04659dcee293"));
...
unsigned char pchMessageStart[4] = { 0xff, 0xc2, 0xb8, 0xdf }; // Experiencecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ExperiencecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ExperiencecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ExperiencecoinMiner(CWallet *pwallet)
{
    printf("ExperiencecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("experiencecoin-miner");
    ...
    try { loop {
        ...
        printf("Running ExperiencecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ExperiencecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ExperiencecoinMiner, pwallet));
}
