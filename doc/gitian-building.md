...
*Setup instructions for a Gitian build of Experiencecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Experiencecoin
...
- [Building Experiencecoin Core](#building-experiencecoin-core)
...
Clone the git repositories for experiencecoin and Gitian.
...
in the experiencecoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Experiencecoin Core
----------------

To build Experiencecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the experiencecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/experiencecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/experiencecoin/experiencecoin
...
For example:
```bash
URL=https://github.com/thrasher-/experiencecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit experiencecoin=${COMMIT} --url experiencecoin=${URL} ../experiencecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit experiencecoin=${COMMIT} --url experiencecoin=${URL} ../experiencecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit experiencecoin=${COMMIT} --url experiencecoin=${URL} ../experiencecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the experiencecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../experiencecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/experiencecoin
SIGPATH=/some/root/path/experiencecoin-detached-sigs

./bin/gbuild --url experiencecoin=${BTCPATH},signature=${SIGPATH} ../experiencecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/experiencecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/experiencecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/experiencecoin-osx-build.assert
```
