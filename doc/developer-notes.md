...
- ThreadSocketHandler : Sends/Receives data from peers on port 7300.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7400 for connections and services them.

- BitcoinMiner : Generates experiencecoins (if wallet is enabled).
...
However, in open source software such as Experiencecoin Core, where everyone uses
...
pay attention to for reviewers of Experiencecoin Core code.

General Experiencecoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Experiencecoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Experiencecoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Experiencecoin Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:experiencecoin/experiencecoin.git
