...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7400
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7300
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17400
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17300
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 7400
	%{_sbindir}/semanage port -d -p tcp 7300
	%{_sbindir}/semanage port -d -p tcp 17400
	%{_sbindir}/semanage port -d -p tcp 17300
	for selinuxvariant in %{selinux_variants}; do
